...
- 传输加密：支持 HTTPS（服务端证书 + 客户端 CA 校验证书）。
- 主机名校验：HTTPS 模式支持 `SNI + 证书主机名校验`。
...
mkdir -p build
g++ -std=c++17 -O2 -Iinclude -I"$CONDA_PREFIX/include" src/config.cpp src/common.cpp src/http.cpp src/log.cpp src/storage.cpp src/license_server_main.cpp -L"$CONDA_PREFIX/lib" -lsqlite3 -lssl -lcrypto -o build/license_server
g++ -std=c++17 -O2 -Iinclude src/config.cpp src/common.cpp src/http.cpp src/log.cpp src/license_client_main.cpp -lssl -lcrypto -o build/license_client
g++ -std=c++17 -O2 -Iinclude -I"$CONDA_PREFIX/include" src/config.cpp src/common.cpp src/http.cpp src/log.cpp src/storage.cpp tests/test_duccore.cpp -L"$CONDA_PREFIX/lib" -lsqlite3 -lssl -lcrypto -o build/duccore_tests
```
...
- `--tls-cert`：服务端 TLS 证书（PEM）。
- `--tls-key`：服务端 TLS 私钥（PEM）。
...
- `--cache`：本地授权缓存路径。
- `--grace`：离线宽限时长（秒）。
- `--tls-ca`：客户端信任的 CA 或服务端自签证书（PEM）。
- `--tls-server-name`：证书主机名校验值（例如 `duc-demo.local`）。
...
```bash
# 启动 HTTPS 服务端
./build/license_server --port 8088 --duration 120 --tls-cert ./certs/server.crt --tls-key ./certs/server.key

# 客户端激活/心跳（带证书校验）
./build/license_client activate --host 127.0.0.1 --port 8088 --machine my-lora-node --cache ./license_cache.txt --tls-ca ./certs/server.crt --tls-server-name duc-demo.local
./build/license_client run --host 127.0.0.1 --port 8088 --machine my-lora-node --cache ./license_cache.txt --grace 20 --tls-ca ./certs/server.crt --tls-server-name duc-demo.local
```
...
13. HTTPS 场景（服务端证书 + 客户端证书校验）成功/失败路径验证。
14. HTTPS 证书主机名不匹配时拒绝连接。
