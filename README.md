...
- 结构化日志：服务端输出包含 `level` 和 `request_id` 的 JSON 日志。
...
mkdir -p build
g++ -std=c++17 -O2 -Iinclude src/common.cpp src/http.cpp src/log.cpp src/license_server_main.cpp -o build/license_server
g++ -std=c++17 -O2 -Iinclude src/common.cpp src/http.cpp src/log.cpp src/license_client_main.cpp -o build/license_client
g++ -std=c++17 -O2 -Iinclude src/common.cpp src/http.cpp src/log.cpp tests/test_duccore.cpp -o build/duccore_tests
```
